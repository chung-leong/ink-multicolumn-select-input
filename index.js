  ...
  useInput((input, key) => {
    ...
    if (input === 'k' || key.upArrow) {
      ...
      if (newIndex >= items.length) {
        ...
        const currentColumn = Math.floor(selectedIndex / limit);
        const lastColumn = Math.floor((items.length - 1) / limit);
    ...
    if (newIndex >= 0 && newIndex < items.length) {
      ...
      setColumnOffset(calculateColumnOffset(newIndex, columnCount, limit, columnOffset));
...
function calculateColumnOffset(index, count, limit, offset = 0) {
  const columnIndex = Math.floor(index / limit);
